...
{
  Serial.println("+WriteStringToEEPROM()");
  int iEepromLocation = EEPROM.length() - 18;

  String currentString = s_currentString + s_availableChars[s_currentCharIndex];
  
  for (int i = 0; i < currentString.length(); i++)
  {
    EEPROM.write(iEepromLocation, currentString[i]);
    iEepromLocation++;
  }

  while (iEepromLocation < EEPROM.length())
  {
    EEPROM.write(iEepromLocation, 0);
    iEepromLocation++;
  }
  Serial.println("-WriteStringToEEPROM()");
}
...
{
  Serial.println("+ReadStringFromEEPROM()");
  s_currentString = "";

  if (EEPROM.read(EEPROM.length() - 1) != 0)
  {
    Serial.println("ReadStringFromEEPROM() -- No string stored to EEPROM!");
    Serial.println("-ReadStringFromEEPROM()");
    return;
  }
  
  for (int iEepromLocation = EEPROM.length() - 18; iEepromLocation < EEPROM.length(); iEepromLocation++)
  {
    char currentChar = EEPROM.read(iEepromLocation);
    if (currentChar == 0)
    {
      break;
    }
    s_currentString = s_currentString + currentChar;
  }
...
void setup()
{
  ...
  for (int iAddress = EEPROM.length() - 18; iAddress < EEPROM.length(); iAddress++)
  {
    byte value = EEPROM.read(iAddress);
